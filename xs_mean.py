...
for i in range(n_feed):
    for j in range(n_feed):
        try:
            filepath = 'spectra/xs_feeds_co6_half_0_map_full_%02i_full_%02i.h5' % (i+1, j+1)
        ...
        if abs(chi2[i,j]) < 5.0 and not np.isnan(chi2[i,j]): # and i != j:
...
plt.savefig('xs_grid_full.png', bbox_inches='tight')
...
plt.savefig('xs_mean_full.png', bbox_inches='tight')
